...
const Sidebar: React.FC = () => {
    ...
    return (
        ...
        >
            {/* Background glow nhẹ phía trên */}
            <div className="absolute top-0 left-0 w-full h-24 bg-gradient-to-b from-[#FF1493]/5 to-transparent pointer-events-none" />

            {/* Logo & Toggle */}
            <div className={`flex items-center justify-between px-3 mb-10 shrink-0 relative z-10 ${expanded ? 'gap-3' : 'justify-center'}`}>
            ...
            <div className="mt-4 pt-4 border-t border-white/[0.07] shrink-0 relative z-10">
                {expanded ? (
                    ...
                    <div className="flex flex-col items-center gap-2">
                        <Link
                            to="/profile/me"
                            className="w-10 h-10 rounded-full bg-gradient-to-br from-[#DCA3C8] to-[#FF1493] p-[2px] hover:shadow-[0_0_15px_rgba(255,20,147,0.4)] transition-all duration-300"
                            title={fullName}
                        >
                            <div className="w-full h-full bg-[#1C1C1E] rounded-full flex items-center justify-center overflow-hidden">
                                {avatarUrl && !imgError ? (
                                    ...
                                    <span className="text-[#EBEBF5] font-bold text-xs">
                                        {fullName?.charAt(0)?.toUpperCase() || 'U'}
                                    </span>
                                )}
                            </div>
